...
def extract_job_preferred_skills(job_json):
    """
    For preferred skill requirements, we pull from job_json["preferred"]["hard_skills"],
    similar to how we used job_json["mandatory"]["hard_skills"] for mandatory.
    """
...
def aggregate_best_entries(resume_skills, job_required_skill):
    ...
    """
    by_job_id = {}  # Map of job_id -> {"sim": float, "years": float}
...
def compute_single_preferred_requirement_score(
    ...
    3) Iterates in descending similarity order:
       a) If coverageUsed=0 and we encounter an empty job_id skill:
          ...
          - Else skip it
       b) If coverageUsed=0 and we encounter a real job_id skill => start real coverage accumulation
          until min_years_required is met or exhausted
       c) Once we pick coverage from empty or real, we do NOT mix them.

    Returns the final weighted similarity score for that requirement.
    """
    print("\n=== Processing Single Preferred Skill Requirement ===")
    ...
    print("\n-- Sorted Preferred Skill Entries (desc by sim) --")
    ...
    for item in best_entries:
        ...
        if coverage_used >= min_years_required:
            print("Already met preferred coverage. Break.")
            break
        ...
        if coverage_mode is None:
            ...
            if coverage_mode == "empty":
                # We used an empty job_id skill that meets coverage alone => do not add more
                print(
...
def calculate_preferred_skill_match_score(job_json, resume_json):
    """
    Iterates over each preferred skill requirement in the job JSON,
    calls compute_single_preferred_requirement_score, and returns their average.
    """
    print("\n======================")
    print("BEGIN: calculate_preferred_skill_match_score")
    print("======================")

    job_skills = extract_job_preferred_skills(job_json)
    if not job_skills:
        print("No preferred skill requirements found. Returning None.")
        return None

    resume_skills = extract_resume_skills(resume_json)
    requirement_scores = []
    for idx, req in enumerate(job_skills, start=1):
        print(f"\n********** PREFERRED REQUIREMENT #{idx} **********")
        job_required_skill = req.get("skill", [])
        min_years_required = req.get("minyears", [0])[0]

        score_for_this_req = compute_single_preferred_requirement_score(
            resume_skills, job_required_skill, min_years_required
        )
        print(
            f"[PREFERRED REQUIREMENT #{idx}] Weighted Similarity Score = {score_for_this_req:.3f}"
        )
        requirement_scores.append(score_for_this_req)

    overall_skill = safe_average(requirement_scores)
    if overall_skill is None:
        print("No valid preferred scores found at all. Returning None.")
        return None
    else:
        print(f"\n=> Overall Preferred Skill Match Score = {overall_skill:.3f}")
        return overall_skill


def calculate_preferred_skill_score(job_json, resume_json):
    """
    Returns a dictionary with the "preferred_skill_score" for a single job/resume pair.
    """
    score = calculate_preferred_skill_match_score(job_json, resume_json)
...
def calculate_preferred_skill_scores(job_json_list, resume_json):
    ...
    mapping each job's job_id to its preferred skill score.
    """
    results = {}
    print("\n##################################")
    print("BEGIN: calculate_preferred_skill_scores")
    print("##################################")

    for i, job_json in enumerate(job_json_list, start=1):
        job_id = job_json.get("job_id", f"job_{i}")
        print(f"\n======================")
        print(f"PROCESSING JOB_ID: {job_id} (Preferred Skills)")
        print(f"======================")
