...
def get_background_match_score(
    ...
    then we sum the candidate's total background years. If the total meets or exceeds the minimum required years,
    returns 1.0; otherwise, 0.0.
    Otherwise, for each candidate background entry:
      - Compare each candidate term (from "background") against each job requirement group.
      - If a group has multiple elements, compute the average similarity.
      - For the candidate entry, take the maximum similarity.
      - If that maximum meets or exceeds the threshold, add the candidate’s years weighted by that maximum.
...
def get_industry_match_score(
    ...
    For each candidate background entry:
      - Compare each candidate industry (from "industry") against each required industry term.
      - For each entry, take the maximum similarity.
...
# ---------------------------
# Mandatory Background Scoring for a Single Job
# ---------------------------
def calculate_mandatory_background_score(job_json, resume_json, threshold=0.6):
    ...
    for req in job_req:
        ...
        total_candidate_years = sum(
            entry.get("years", 0) for entry in candidate_background
        )
...
def calculate_mandatory_background_scores(job_json_list, resume_json, threshold=0.6):
    """
    Accepts a list of job JSON objects and returns a dictionary mapping each job's
    job_id to its mandatory background scores.
    The returned dictionary uses the job's "job_id" as the key and a dictionary with keys:
         "mandatory_background_score" and "mandatory_industry_score"
    as the value.
    """
    results = {}
    for job_json in job_json_list:
        mand_bg, mand_ind = calculate_mandatory_background_score(
...
#     # Example usage:
#     # Replace these with your actual list of job JSON objects (each with a "job_id")
#     # and your resume JSON object.
#     job_json_list = []  # List of job JSON objects
#     resume_json = {}  # Your resume JSON object
#     results = calculate_mandatory_background_scores(
#         job_json_list, resume_json, threshold=0.6
#     )
#     # The returned dictionary will have keys equal to each job's "job_id" and values like:
#     # { "mandatory_background_score": score, "mandatory_industry_score": score }
#     print("Mandatory Background Scores for Jobs:")
#     print(results)
